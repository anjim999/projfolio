...
function generateWebDevProject(skills = [], interests = [], techStack = [], durationStr = "") {
  return {
    ...
    features: [
      `Authentication & role-based access (users, admins, ${interests[0]?.toLowerCase() === "ecommerce" ? "vendors" : "contributors"
      })`,
      `Core feature: ${interests[0] === "social"
...
async function generateOfflineSuggestions({
  skills = [],
  level = "Intermediate",
  interests = [],
  techStack = [],
  ...
  if (isML && !isWeb) {
    ...
    suggestions.push({
      ...
      learningOutcomes: [
        ...
        "Use of native device features (camera, location)",
      ],
      duration: durationStr,
      goal: goalStr,
    });
